...
newObj_["tgh::::::jklt"+k]=""
    newObj_["textBoxSizeX2"+k] = node.value.size.x
newObj_["textBoxSizeY2"+k] = node.value.size.y
    if(node.level == 9){
...
console.log(newObj_["textBoxSizeX2"+(k)]/(newObj["fontSize"+(k)]/node.value.style.fontSize))
var _primId = api.create("textShape", newObj_["name"+k]+"----"+(ii+jj));
api.set(_primId,{"text": node.value.characters, "horizontalAlignment": textAlign, "verticalAlignment" : node.value.style.textAlignVertical, "textBoxSize.x": node.value.size.x, "textBoxSize.y": node.value.size.y, "fontSize": newObj["fontSize"+k], "textBoxSize.x": newObj_["textBoxSizeX2"+(k)]/(node.value.style.fontSize/newObj["fontSize"+(k)]), "textBoxSize.y": newObj_["textBoxSizeY2"+(k)]/(node.value.style.fontSize/newObj["fontSize"+(k)])})
