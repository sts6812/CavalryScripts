function vector2( node, newObj, newObj_, ii, jj, k, path, obj, compWidth, compHeight){
...
newObj_["x"+k]=(node.value.relativeTransform[0][2])
    if(node.level == 9){
       newObj_["x"+k]=(node.value.absoluteBoundingBox.x)
        newObj_["xPos"+k] = (((newObj_["x"+k]-(compWidth/2)+(newObj_["xWidth"+k])))-xParDiff)-newObj_["xWidth"+k]
        //newObj_["xPos"+k] = (newObj_["x"+k]+(compWidth/2)+(newObj_["xWidth"+k]))-compWidth
       newObj_["y"+k]=(node.value.absoluteBoundingBox.y);
        newObj_["yPos"+k] = ((newObj_["y"+k]-(compHeight/2)+(newObj_["yHeight"+k]))*-1)+yParDiff
...
newObj_["stagger"+k]=(ii+jj)
        ...
        newObj_["name"+k]=(node.value.name)
            newObj_["xPos"+k] = ((newObj_["x"+k]-(compWidth/2))+(newObj_["xWidth"+k]))-(newObj_["xWidth"+k])
//newObj_["xPos"+k]= (newObj_["x"+k]-(compWidth/2)+(newObj_["xWidth"+k]/2))
 
    newObj_["yPos"+k]= (((newObj_["y"+k]-(compHeight/2))*-1)-newObj_["yHeight"+k])
    }

//var mypath = api.createEditable(path, newObj_["name"+k]);
  //  api.set(mypath, {"pivot.y": (node.value.size.y)*-1})
//path.close()
//newObj_["layer"+k]=(mypath)
